...
from env import *
from utils import pb_predict_mask, load_test_image, load_mask, pct_to_label, R_squared, visualize
tf.logging.set_verbosity(tf.logging.ERROR)

model_path = "./models/model_sky_new.pb"
...
for img_path, mask_path in zip(img_paths, mask_paths):
    ...
    mask = pb_predict_mask(sess, img) < 0.5
    ...
    if not path.exists('./results_sky'):
        makedirs('./results_sky')
    visualize(img, mask, path = "./results_sky/"+img_path.split("/")[-1].split(".")[0]+"_pred.jpg" )
    j+=1
